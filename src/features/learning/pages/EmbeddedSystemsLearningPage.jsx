import { BookOpen, ChevronLeft, ChevronRight, CircleHelp, Cpu, PlayCircle, ShieldCheck } from "lucide-react";
...
import EmbeddedSystemsChapterFourLesson from "./EmbeddedSystemsChapterFourLesson";
import EmbeddedSystemsChapterFiveLesson from "./EmbeddedSystemsChapterFiveLesson";
import EmbeddedSystemsChapterSixLesson from "./EmbeddedSystemsChapterSixLesson";
import EmbeddedSystemsChapterSevenLesson from "./EmbeddedSystemsChapterSevenLesson";
import EmbeddedSystemsChapterEightLesson from "./EmbeddedSystemsChapterEightLesson";
import EmbeddedSystemsChapterNineLesson from "./EmbeddedSystemsChapterNineLesson";
import EmbeddedSystemsChapterTenLesson from "./EmbeddedSystemsChapterTenLesson";

const chapters = courseContent["embedded-systems"]?.chapters || [];

export default function EmbeddedSystemsLearningPage() {
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <main className="col-span-12 space-y-6 lg:col-span-9">
            <section className="rounded-3xl bg-white p-6 shadow-lg">
              ...
              <div className="flex aspect-video items-center justify-center rounded-2xl bg-gradient-to-br from-slate-950 via-blue-950 to-indigo-950 p-8 text-center text-white">
                <div><Cpu className="mx-auto text-blue-300" size={64} /><h3 className="mt-5 text-2xl font-bold">Video lesson coming soon</h3><p className="mt-3 text-blue-100">The administrator can add the YouTube lesson for this chapter below.</p></div>
              </div>
              <ChapterVideoManager chapter={chapter.id} />
            </section>

            {chapter.id === 1 ? <EmbeddedSystemsChapterOneLesson /> : chapter.id === 2 ? <EmbeddedSystemsChapterTwoLesson /> : chapter.id === 3 ? <EmbeddedSystemsChapterThreeLesson /> : chapter.id === 4 ? <EmbeddedSystemsChapterFourLesson /> : chapter.id === 5 ? <EmbeddedSystemsChapterFiveLesson /> : chapter.id === 6 ? <EmbeddedSystemsChapterSixLesson /> : chapter.id === 7 ? <EmbeddedSystemsChapterSevenLesson /> : chapter.id === 8 ? <EmbeddedSystemsChapterEightLesson /> : chapter.id === 9 ? <EmbeddedSystemsChapterNineLesson /> : chapter.id === 10 ? <EmbeddedSystemsChapterTenLesson /> : <ChapterPlaceholder chapter={chapter} />}
            ...
            <nav className="flex flex-col justify-between gap-4 sm:flex-row">
              ...
              {next ? <Link to={chapterPath(next.id)} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-3 font-semibold text-white hover:bg-blue-700">Chapter {next.id}: {next.title} <ChevronRight size={18} /></Link> : <Link to="/courses/embedded-systems" className="inline-flex rounded-xl bg-emerald-600 px-5 py-3 font-semibold text-white hover:bg-emerald-700">Course Overview</Link>}
            </nav>
          </main>
        </div>
      </div>
...
function ChapterPlaceholder({ chapter }) {
  return (
    <article className="rounded-3xl bg-white p-6 shadow-lg md:p-8">
      ...
      <p className="mt-4 leading-8 text-slate-600">This chapter framework is ready. Its complete theory, diagrams, firmware examples, debugging guidance, practical activity, and chapter quiz will be added when we create this chapter separately.</p>
      <div className="mt-7 grid gap-4 md:grid-cols-2">
        ...
        <Feature icon={ShieldCheck} title="Safe hardware practice" text="Practical work will include power, voltage, ESD, current-limit and connection checks before energising a circuit." />
...
function chapterPath(id) {
  return id === 1 ? "/courses/embedded-systems/learn" : `/courses/embedded-systems/learn/chapter-${id}`;
}
