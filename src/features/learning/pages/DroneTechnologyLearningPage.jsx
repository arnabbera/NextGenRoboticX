import { BookOpen, ChevronLeft, ChevronRight, CircleHelp, Plane, PlayCircle, ShieldCheck } from "lucide-react";
...
import DroneChapterOneLesson from "./DroneChapterOneLesson";
import DroneChapterTwoLesson from "./DroneChapterTwoLesson";
import DroneChapterThreeLesson from "./DroneChapterThreeLesson";
import DroneChapterFourLesson from "./DroneChapterFourLesson";
import DroneChapterFiveLesson from "./DroneChapterFiveLesson";
import DroneChapterSixLesson from "./DroneChapterSixLesson";
import DroneChapterSevenLesson from "./DroneChapterSevenLesson";

const chapters = courseContent["drone-technology"]?.chapters || [];

export default function DroneTechnologyLearningPage() {
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <main className="col-span-12 space-y-6 lg:col-span-9">
            <section className="rounded-3xl bg-white p-6 shadow-lg">
              ...
              <div className="flex aspect-video items-center justify-center rounded-2xl bg-gradient-to-br from-slate-950 via-sky-950 to-indigo-950 p-8 text-center text-white">
                <div><Plane className="mx-auto text-sky-300" size={64} /><h3 className="mt-5 text-2xl font-bold">Video lesson coming soon</h3><p className="mt-3 text-sky-100">The administrator can add the YouTube lesson for this chapter below.</p></div>
              </div>
              <ChapterVideoManager chapter={chapter.id} />
            </section>

            {chapter.id === 1 ? <DroneChapterOneLesson /> : chapter.id === 2 ? <DroneChapterTwoLesson /> : chapter.id === 3 ? <DroneChapterThreeLesson /> : chapter.id === 4 ? <DroneChapterFourLesson /> : chapter.id === 5 ? <DroneChapterFiveLesson /> : chapter.id === 6 ? <DroneChapterSixLesson /> : chapter.id === 7 ? <DroneChapterSevenLesson /> : <ChapterPlaceholder chapter={chapter} />}
...
function ChapterPlaceholder({ chapter }) {
  return (
    <article className="rounded-3xl bg-white p-6 shadow-lg md:p-8">
      ...
      <p className="mt-4 leading-8 text-slate-600">This chapter framework is ready. Its complete theory, diagrams, configuration guidance, safety procedures, hands-on activity, and chapter quiz will be added when we create this chapter separately.</p>
      <div className="mt-7 grid gap-4 md:grid-cols-2">
        ...
        <Feature icon={ShieldCheck} title="Safety-first practical work" text="Every practical activity will include propeller, battery, arming, test-area, and fail-safe precautions." />
...
function chapterPath(id) {
  return id === 1 ? "/courses/drone-technology/learn" : `/courses/drone-technology/learn/chapter-${id}`;
}
