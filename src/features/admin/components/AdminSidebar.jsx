import { BookOpen, ChevronLeft, ChevronRight, FolderKanban, LayoutDashboard, Settings2, Wrench } from "lucide-react";
    ...
    <aside className={`${collapsed ? "w-20" : "w-72"} flex min-h-screen flex-col bg-slate-900 text-white shadow-2xl transition-all duration-300`}>
      <div className="border-b border-slate-800 p-5">
        <div className="flex items-center justify-between">
          <div className="min-w-0">{collapsed ? <BrandLogo className="w-10" /> : <div><BrandLogo className="w-44" /><p className="mt-2 text-xs text-slate-400">Administrator Portal</p></div>}</div>
